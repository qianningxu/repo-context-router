...
`prompt` should preserve the actual user request details needed to audit the routing decision. Include reasons for selected folders and files. Do not include raw document contents in the decision file.
...
Read the matching context file before answering or editing when `selected_tree.children` is not empty. The generated file contains `context_tree`, which mirrors `selected_tree` and attaches raw document content at selected file nodes.
